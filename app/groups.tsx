...
export default function GroupsListScreen() {
  ...
  const { groups, updateMemberStatus, blockGroupCreator, deleteGroupForUser, isEventNewForUser } = useGroups();
  ...
  const handleAcceptInvitation = async () => {
    ...
    try {
     ...
     Alert.alert('Invitation Accepted', `You've joined "${selectedInvitation.name}"`);
   } catch (error) {
 ...
 const handleDeclineInvitation = () => {
   ...
   Alert.alert('Invitation Declined', `You've declined the invitation to "${selectedInvitation.name}"`);
 };

 const handleBlockInvitation = () => {
   if (!selectedInvitation) return;
   
   Alert.alert(
     'Block Group and Creator',
     `Are you sure you want to block "${selectedInvitation.name}" and its creator? This action cannot be undone and will permanently delete the group from your view.`,
     [
       { text: 'Cancel', style: 'cancel' },
       {
         ...
         style: 'destructive',
         onPress: () => {
           blockGroupCreator(selectedInvitation.id);
           deleteGroupForUser(selectedInvitation.id);
           setShowInvitationModal(false);
           setSelectedInvitation(null);
           
           Alert.alert('Group Blocked', 'The group and its creator have been blocked.');
         }
       }
     ]
   );
 ...
 const handleDeleteGroup = async () => {
   ...
   if (userStatus === 'declined') {
     Alert.alert(
       'Remove Group',
       `Remove "${selectedGroupForAction.name}" from your list?`,
       [
         { text: 'Cancel', style: 'cancel' },
         {
           text: 'Remove',
   ...
   if (isAdmin) {
     Alert.alert(
       'Delete Group',
       `Are you sure you want to permanently delete "${selectedGroupForAction.name}"? This action cannot be undone and will remove the group for all members.`,
       [
         { text: 'Cancel', style: 'cancel' },
         {
           text: 'Delete Forever',
           ...
           onPress: async () => {
             try {
               console.log(`Admin ${user?.uid} deleting group ${selectedGroupForAction.id}`);
               await deleteGroupForUser(selectedGroupForAction.id);
               setSelectedGroupForAction(null);
               Alert.alert('Group Deleted', 'The group has been permanently deleted for all members.');
             } catch (error) {
     ...
     Alert.alert(
       'Leave Group',
       `Are you sure you want to leave "${selectedGroupForAction.name}"?`,
       [
         { text: 'Cancel', style: 'cancel' },
         {
           text: 'Leave',
           ...
           onPress: async () => {
             try {
               console.log(`Member ${user?.uid} leaving group ${selectedGroupForAction.id}`);
               await deleteGroupForUser(selectedGroupForAction.id);
               setSelectedGroupForAction(null);
 ...
 const handleSilentNotification = () => {
   ...
   Alert.alert('Notification Setting Changed', 'Group notification settings updated.');
   setSelectedGroupForAction(null);
 };
         ...
         <View style={styles.modalContainer}>
           ...
           <View style={styles.modalActions}>
             ...
             <TouchableOpacity
               ...
               onPress={handleBlockInvitation}
             >
               <Text style={styles.modalButtonTextSmall}>Block Group (incl. Group Creator)</Text>
             </TouchableOpacity>
           </View>
         </View>
       </View>
