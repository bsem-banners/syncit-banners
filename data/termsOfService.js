export const termsOfService = `
# Terms of Service

**Effective Date:** [10/07/2025]  
**Last Updated:** [10/07/2025]

## 1. Introduction

Welcome to SynciT ("we," "our," or "us"). These Terms of Service ("Terms") govern your use of the SynciT mobile application and related services (collectively, the "Service"). By accessing or using our Service, you agree to be bound by these Terms.

## 2. Acceptance of Terms

By creating an account, downloading, accessing, or using SynciT, you:
- Confirm you are at least 13 years old (or the minimum age required in your jurisdiction)
- Agree to these Terms and our Privacy Policy
- Represent that you have the authority to enter into this agreement

## 3. Description of Service

SynciT is a group coordination and event planning application that allows users to:
- Create and manage groups with friends and family
- Schedule and coordinate events
- Share calendars within groups
- Send notifications about group activities
- Communicate with group members

## 4. User Accounts and Registration

### 4.1 Account Creation
- You must provide accurate, current, and complete information
- You are responsible for maintaining the security of your account
- You must notify us immediately of any unauthorized access

### 4.2 Account Responsibilities
- You are responsible for all activities under your account
- You may not share your account credentials
- You may not create multiple accounts or impersonate others

## 5. Acceptable Use Policy

### 5.1 Permitted Uses
You may use SynciT for lawful purposes in accordance with these Terms.

### 5.2 Prohibited Uses
You may not:
- Use the Service for illegal activities or to violate any laws
- Harass, abuse, or harm other users
- Share inappropriate, offensive, or harmful content
- Spam or send unsolicited communications
- Attempt to gain unauthorized access to our systems
- Use automated tools to access the Service
- Interfere with the Service's operation
- Share false or misleading information
- Violate intellectual property rights

## 6. Content and Privacy

### 6.1 Your Content
- You retain ownership of content you submit
- You grant us a license to use your content to provide the Service
- You are responsible for ensuring you have rights to share content
- You may delete your content at any time

### 6.2 Privacy
- Your privacy is important to us
- Please review our Privacy Policy for information about data collection and use
- We implement security measures to protect your information

## 7. Groups and Events

### 7.1 Group Management
- Group administrators have control over group settings and membership
- Users can leave groups at any time
- Group administrators may remove members

### 7.2 Event Information
- Event details are shared with group members
- Users are responsible for the accuracy of event information
- We are not responsible for event cancellations or changes

## 8. Communications and Notifications

### 8.1 Service Communications
- We may send you notifications about the Service
- You can manage notification preferences in your settings
- Some communications are necessary for the Service to function

### 8.2 User Communications
- Users may communicate through the Service
- We do not monitor private communications
- Report inappropriate behavior through our reporting features

## 9. Intellectual Property

### 9.1 Our Rights
- SynciT and its features are our intellectual property
- You may not copy, modify, or distribute our software
- Our trademarks and logos are protected

### 9.2 Your Rights
- You retain rights to your original content
- We respect intellectual property rights of others
- Report copyright infringement to us

## 10. Disclaimers and Limitations

### 10.1 Service Availability
- We strive for reliable service but cannot guarantee 100% uptime
- The Service is provided "as is" without warranties
- We may modify or discontinue features with notice

### 10.2 Limitation of Liability
- To the maximum extent permitted by law, we limit our liability
- We are not responsible for user-generated content
- We are not liable for indirect or consequential damages

## 11. Indemnification

You agree to indemnify and hold us harmless from claims arising from:
- Your use of the Service
- Your violation of these Terms
- Your violation of others' rights

## 12. Termination

### 12.1 Termination by You
- You may delete your account at any time
- Termination does not affect accrued rights or obligations

### 12.2 Termination by Us
- We may suspend or terminate accounts for Terms violations
- We may terminate the Service with reasonable notice
- We reserve the right to refuse service

## 13. International Users

### 13.1 Global Service
- SynciT is available internationally
- You must comply with local laws in your jurisdiction
- Some features may not be available in all regions

### 13.2 Jurisdiction-Specific Terms

**For EU Users:**
- You have additional rights under EU consumer protection laws
- Data processing is governed by GDPR
- You may have the right to withdraw consent for data processing

**For UK Users:**
- UK consumer protection laws apply
- Data processing follows UK GDPR requirements
- You have rights regarding your personal data

## 14. Changes to Terms

- We may update these Terms periodically
- Significant changes will be communicated with 30 days notice
- Continued use after changes constitutes acceptance
- Previous versions are available upon request

## 15. Dispute Resolution

### 15.1 General
- We encourage resolving disputes informally first
- Contact us at [support email] for assistance

### 15.2 For US Users
- Disputes are governed by [State] law
- Disputes may be resolved through binding arbitration
- Class action waivers may apply where legally permitted

### 15.3 For EU/UK Users
- You may have additional rights under consumer protection laws
- You may access out-of-court dispute resolution platforms
- Nothing in these Terms affects your statutory rights

## 16. Miscellaneous

### 16.1 Entire Agreement
These Terms, along with our Privacy Policy, constitute the entire agreement.

### 16.2 Severability
If any provision is unenforceable, the remainder remains in effect.

### 16.3 No Waiver
Our failure to enforce any right does not waive that right.

### 16.4 Assignment
We may assign these Terms; you may not without our consent.

## 17. Contact Information

For questions about these Terms, contact us at:

**Email:** [legal@syncit.app]  
**Address:** [Your Company Address]  
**Support:** [support@syncit.app]

---

*These Terms of Service are designed to protect both SynciT and our users while ensuring compliance with applicable laws in the United States, United Kingdom, and European Union.*
`;